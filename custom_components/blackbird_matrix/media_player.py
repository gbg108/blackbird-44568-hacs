    ...
    MEDIA_PLAYER_PLATFORM_SCHEMA.extend(
        {
            ...
            vol.Optional(CONF_BAUD): vol.All(
                vol.Coerce(int),
    ...
    if port is not None:
        if model == MODEL_44568:
            try:
                ...
                _LOGGER.error("Error connecting to Blackbird 44568 at %s: %s", port, exc)
                return
            ...
            try:
                ...
                _LOGGER.error("Error connecting to Blackbird 39670 at %s: %s", port, exc)
                return
        else:
            try:
...
class BlackbirdZone(MediaPlayerEntity):
    ...
    def select_source(self, source: str) -> None:
        """Set input source."""
        _LOGGER.warning("Blackbird: select_source called zone=%d source=%r", self._zone_id, source)
        if source not in self._source_name_id:
            return
        idx = self._source_name_id[source]
        _LOGGER.warning("Blackbird: routing input %d → output (zone) %d", idx, self._zone_id)
